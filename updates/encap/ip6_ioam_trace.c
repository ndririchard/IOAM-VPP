...
ip6_hbh_ioam_trace_data_list_handler (vlib_buffer_t * b, ip6_header_t * ip, ip6_hop_by_hop_option_t * opt)
{
  ...
  if(profile->node_type & IOAM_NODE_DECAP)
  {
    ...
    checkForIntegrity(trace, &rv); //check for integrity
  }
  ...
  //*************************************************************************************************
  u32* buf = malloc (sizeof(u32) * 100); // allocate buffer for the signature value
  int index;
  if (index == MAX_NODE)
    return (-1); // there is no place to strore the inegrity value
  if (profile->node_type & IOAM_NODE_ENCAP){
      // allocate buf 
      &trace->integrity_hdr->signature = malloc(sizeof(u32) * MAX_NODE);
      &trace->integrity_hdr->cp_data = malloc(sizeof(u32) * MAX_NODE);

      index = findFreeIndex(&trace->integrity_hdr->signature);

      char* nn = generateNonce();
      &trace->integrity_hdr->nonce = clib_net_to_host_u32(nn);
      &trace->integrity_hdr->signature_suite = 0x01;
      &trace->integrity_hdr->nonce_len = clib_net_to_host_u8(strlen(nn));
      AES256_ENC(trace->integrity_hdr->nonce, buf);

      &trace->integrity_hdr->cp_data[index] = &trace->integrity_hdr->nonce;
  }

  else if (profile->node_type & IOAM_NODE_TRANSIT){
    index = findFreeIndex(&trace->integrity_hdr->signature);
    u32 wrd = elt + &trace->integrity_hdr->signature[index-1];
    AES256_ENC(wrd, buf);
    &trace->integrity_hdr->cp_data[index] = elt;
  } 
  &trace->integrity_hdr->signature[index] = clib_net_to_host_u32(buf);
  //*************************************************************************************************
  return (rv);
}
