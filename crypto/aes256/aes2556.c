...
void AES256MainECB(u8 *key, u8 *data, u32 datalen, u8 *returndata, u32 *returnlen, bool encrypt){
  u32 temp = datalen;
  u32 numbBlocks = ceil(datalen/16.0);
  u32 dataBytes = 0;
  u32 returnBytes = 0;
  u32 i,x,y;
...
void AES256MainCBC(u8 *key, u8 *data, u8 *iv, u32 datalen, u8 *returndata, u32 *returnlen, bool encrypt){
  u32 temp = datalen;
  u32 numbBlocks = ceil(datalen/16.0);
  u32 dataBytes = 0;
  u32 returnBytes = 0;
...
void AES256MainCTR(u8 *key, u8 *data, u8 *iv, u32 datalen, u8 *returndata, u32 *returnlen, bool encrypt){
  u32 temp = datalen;
  u32 numbBlocks = ceil(datalen/16.0);
  u32 dataBytes = 0;
  u32 returnBytes = 0;
...
u8 * _Pru32_Word(AES256_Word word){
  static u8 buffer[10];
  spru32f(buffer, "%02x%02x%02x%02x",word.x[0],word.x[1],word.x[2],word.x[3]);
  return buffer;
}

u8 * _Pru32_State(AES256_State state){
  static u8 buffer[40];
  spru32f(buffer, "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x",
